...
class ramAssetUIClass:
    ...
    def importAsset(self,*args):
        ...
        if assetSelection==None:
            ...
            if assetType!='':
                veRegCore.importAsset(assetType=assetType, assetName=assetSelection)
        return
            ...
            for chk in veRegCore.listAssetTable():
                if str(chk[1])==assetSelection:
                    ...
                    if repVar=='OK':
                        ...
                        veRegCore.updateAssetRecord(name=assetSelection,\
                                                       description=newDesc)
    ...
    def populateViewLog(self,*args):
        ...
        logNotes=None
        for chk in veRegCore.listLogTable():
    ...
    def changeKeywordFun(self,*args):
        ...
        if repVar!='CANCEL':
            veRegCore.updateAssetRecord(name=str(assetNameVar),keyword=str(cmds.promptDialog(q=True,tx=True)))
        self.listLegacyAndPicture()
        return
    ...
    def downloadLegacy(self,mode ):
        ...
        if repVar=='Proceed':
            ...
            try:
                veRegCore.downloadAssetLegacy(name=assetNameVar,assetType=assetTypeVar,assetTarget=mode)
    ...
    def downloadAsset(self,*args):
        ...
        if repVar!='CANCEL':
            ...
            try:
                veRegCore.downloadAsset(name=assetNameVar,assetType=assetTypeVar,assetTarget=mode)
            except Exception as e:
                cmds.confirmDialog(icn='warning', t='Error', m='File did not exists', button=['Ok'])
                raise StandardError, str(e)
        #refresh
    ...
    def refreshFun(self, *args):
        import veRegAssetManager
        reload (veRegAssetManager)
        return
    ...
    def registerNewAssetRecord(self,*args):
        ...
        if repVar=='Proceed':
            ...
            try:
                #register record
                veRegCore.registerAssetRecord(name=assetNewNameVar,\
                                                 keyword=assetNewKeyVar,\
                                                 type=assetNewOptionVar,description=assetNewDescriptionVar)

                #upload current file
                veRegCore.uploadAsset(name=assetNewNameVar,assetType=assetNewOptionVar,\

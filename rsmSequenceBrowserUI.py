...
class rsmSequenceBrowser:
    ...
    def loadLegacy(self, *args):
        ...
        rsmSequenceCore.loadLegacy(eps=episode, shot=shot, file=file)
        return
    ...
    def postNotes(self,mode):
        if mode==0:
            ...
            rsmSequenceCore.addNotes(eps=episode, shot=shot, title=titleVar, notes=messageVar)
            self.popNotes()
        return
    ...
    def viewLog(self,*args):
        ...
        for chk in rsmSequenceCore.listLog(eps=episode, shot=shot):
    ...
    def downloadSeq(self, *args):
        ...
        if cmds.file(q=True, modified=True):
            ...
            if repVar == 'Skip': rsmSequenceCore.downloadSeq(eps=episode, shot=shot)
            elif repVar == 'Save': cmds.file(s=True)
        else:
            rsmSequenceCore.downloadSeq(eps=episode, shot=shot)
        return

    def popEps(self):
        cmds.textScrollList('episodeList', e=True, ra=True)
        for item in rsmSequenceCore.listEps():cmds.textScrollList('episodeList', e=True, a=str(item))
        return
    ...
    def popShot(self):
        ...
        if selEps is not None:
            selEps=selEps[0]
            for item in rsmSequenceCore.listShot(eps=selEps):
    ...
    def popInfoAndNotes(self):
        ...
        #get shotInfo data
        shotInfo = rsmSequenceCore.getShotInformation(eps=episode, shot=shot)
    ...
    def popNotes(self):
        ...
        #get notes data
        noteList = rsmSequenceCore.listNotes(eps=episode, shot=shot)
    ...
    def popLegacy(self):
        ...
        cmds.textScrollList('legacyList', e=True, ra=True)
        parsed = rsmSequenceCore.listLegacy(eps=episode, shot=shot)
        parsed.reverse()
        for item in parsed:
